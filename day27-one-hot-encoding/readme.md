...
**Math (Collinearity)**
- For $k$ categories with one-hot matrix $H \in \{0,1\}^{n\times k}$ and row sums $\sum_j H_{ij}=1$, columns are linearly dependent: $\sum_j H_{ij}=1$. Dropping one column yields full rank for linear models.
...
Integrate in ColumnTransformer + Pipeline:
```python
...
df['City_top'] = df['City'].where(df['City'].isin(top), 'Other')
```
...
One-Hot Encoding is the go-to technique for nominal data. Always remember to use `drop='first'` to keep your linear models happy and avoid multicollinearity.
 Also set `handle_unknown='ignore'` in pipelines to safely handle unseen categories at inference.
