...
**Pros/Cons**
- Pros: Prevents leakage; reproducible training; unified hyperparameter search; deployable artifact.
- Cons: Debugging internals requires care; transforms must be deterministic; serialization size can be large with many steps.
...
param_grid = {
    'imputer__simpleimputer__strategy': ['mean','median'],
...
## 5. Summary
Pipelines are the mark of a mature Data Scientist. Use them with `GridSearchCV` to tune preprocessing and model hyperparameters together, and persist the final pipeline for deployment.
