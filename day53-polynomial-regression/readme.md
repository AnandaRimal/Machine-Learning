...
High-degree polynomials can explode outside the training range. Combining polynomial features with **Ridge/Lasso/ElasticNet** stabilizes the fit and reduces overfitting.
...
### 4.4 Pipelines and Scaling
Polynomial terms amplify scale differences. Use a `Pipeline` with scaling to keep coefficients well-behaved.
```python
from sklearn.pipeline import make_pipeline
from sklearn.preprocessing import StandardScaler

model = make_pipeline(PolynomialFeatures(3), StandardScaler(with_mean=False), LinearRegression())
