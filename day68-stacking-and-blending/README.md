...
### Bias–Variance View and Why It Works
- Base learners supply diverse inductive biases; meta‑learner combines them to reduce variance while correcting systematic bias.
- Meta‑learner learns a mapping in the space of predictions, often simpler than original feature space.

### Classification Math (Probabilities)
If base learners produce probabilities $p_i(y=1|\mathbf{x})$, a logistic meta‑learner fits
$$ \hat{y} = \sigma\big(w_0 + \sum_i w_i\,p_i(y=1|\mathbf{x})\big) $$
with log‑loss.

### Regression Math (Stacking)
For regression bases $h_i(\mathbf{x})$, a ridge meta‑learner solves
$$ \min_{\mathbf{w}} \|\mathbf{y} - w_0 - \sum_i w_i h_i(\mathbf{X})\|_2^2 + \lambda\|\mathbf{w}\|_2^2 $$
to stabilize weights.

### Blending Objective
Train bases on a holdout split $(X_{tr},y_{tr})$, generate meta‑features on $(X_{val},y_{val})$, then optimize the same objectives as above on $X_{val}$; predict test by passing base predictions through meta‑learner.
...
![Meta-learner combining calibrated probabilities (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=Stacking+Meta-Learner+Diagram+by+Nano+Banana+Model)
...
![Cross-validation OOF pipeline with leakage guard (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=OOF+Pipeline+by+Nano+Banana+Model)
...
![Train/Validation/Test split schematic (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=Blending+Split+Schematic+by+Nano+Banana+Model)
...
![Pros/Cons matrix of stacking vs blending (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=Stacking+vs+Blending+Pros%2FCons+by+Nano+Banana+Model)
...
Mathematically, minimize a convex combination error:
$$ \min_{\mathbf{w}\ge 0,\,\sum w_i=1} \; \mathcal{L}\Big(\sum_i w_i \hat{y}_i, y\Big) $$
...
![Feature augmentation at meta-level (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=Meta-Feature+Engineering+by+Nano+Banana+Model)
...
![K-fold OOF stacking in competitions (to be generated by Nano Banana Model)](https://via.placeholder.com/800x400?text=K-Fold+OOF+Stacking+by+Nano+Banana+Model)
...
## ✅ Advantages and ⚠️ Disadvantages

- **Advantages**:
    - Combines complementary strengths of diverse models, improving generalization.
    - Meta‑learner can correct systematic errors in bases.
    - Works with probabilities for calibrated decisions and threshold tuning.
- **Disadvantages**:
    - Computationally expensive (k‑fold × models × layers).
    - High risk of data leakage without strict OOF discipline.
    - Harder to interpret; requires careful validation and reproducibility.

## 🧭 Where It Shines vs When to Avoid
- **Use**: Competitions, production systems demanding top accuracy, heterogeneous feature spaces.
- **Avoid**: Tiny datasets, strict latency constraints, when transparency is mandatory and a single model suffices.
